...
export default function KanbanBoard() {
  ...
  const handleDemote2 = (taskid, newstate) => {
  ...
  const handlePromote2 = (taskid, newstate) => {
    setNewState(newstate);
    setSelectedTaskId(taskid);
    setKey((prevKey) => prevKey + 1); // Update the key to trigger a re-render
    setShowModal(true);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        if (isTaskFormSubmitted || isPlanFormSubmitted) {
          const getAllTasksResponse = await axios.get(
            `/apps/${params.appacronym}/tasks`
          );
          if (getAllTasksResponse.data.data) {
                    ...
                    {state.tasks.data
                      ...
                      .map((task) => (
                        <TaskCard
                          ...
                          handleDemote2={handleDemote2}
                          handlePromote2={handlePromote2}
                        />
                      ))}
                  </div>
                  <div
